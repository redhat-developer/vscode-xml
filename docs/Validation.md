...
Another useful option are the rewrite rules. Given the same xml as before but now with `xmlns="http://foo.web/footypes.xsd"` can be adjusted to use a local `types.xsd` as follows:
